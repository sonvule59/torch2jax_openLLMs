...
class SomeLayer(nn.Module):
    ...
    def setup(self):
        self.key = self.make_rng('rng')  # Use a separate RNG stream
...
def main():
    ...
    output = layer.apply(params, input_tensor, rngs={'rng': key})  # Apply the initialized layer to the tensor
    ...
    heatmap = transforms.Resize(image.size)(output_img)  # Resize the heatmap image
