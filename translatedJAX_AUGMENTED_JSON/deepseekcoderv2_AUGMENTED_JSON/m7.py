...
class SomeLayer(nn.Module):
    ...
    def setup(self):
        self.key = self.make_rng('rng')  # Create a new RNG for this operation
        
    def __call__(self, x):
        subkey, _ = random.split(self.key)  # Split the key for a new operation
        return nn.relu(nn.Dense(self.features)(x))  # Use a dense layer with relu activation

def generate_random_tensor(shape, dtype=jnp.float32, key=None):  # Explicit dtype and PRNGKey
    if key is None:
